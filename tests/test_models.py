...
def test_dms():
    my_coordinate = Coordinate("ip", datetime.fromtimestamp(1000), 15.2, -3.1)
...
def test_json():
    my_coordinate = Coordinate.from_json(
        "{" '"ip":"ip",' '"time":"1000",' '"latitude":"15.2",' '"longitude":"-3.1"' "}"
    )
    assert my_coordinate == Coordinate("ip", datetime.fromtimestamp(1000), 15.2, -3.1)
